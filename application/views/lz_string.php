...
<html>
	<head>
		...
		<script src="<?=base_url('assets/lz-string-1.4.4.js') ?>"> </script>
		<script src="<?=base_url('assets/dbarid.js') ?>"> </script>
		<title>LZ String</title>
	</head>
	<body>
		<div><button onclick="compresss()">Compress</button></div>
		<textarea></textarea>
<script>
function compresss(){
	var ret='var dbArId=[\n';
	for (i = 0; i < dbArId.length; i++) {
		ret += '"'+(LZString144.compress(dbArId[i]))+'",\n';
	}
	ret += ']';
	$('textarea').val(ret);
}
